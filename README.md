...
### Android command-line test resources

When JNI is not initialized, the metadata loader uses `Foundation.Bundle.module`
so SwiftPM resolves the resource layout generated by the selected build system.
Inside an Android app, loading continues through Skip's Android bundle and asset
URL handling.

The Android test runner must copy the generated resources alongside the
executable. Skip 1.9.4 and 1.9.7 only copy `.resources`; Skip 1.9.11 also transfers
`.bundle` (see [skipstone #277](https://github.com/skiptools/skipstone/pull/277)).
Use a CLI version containing that fix when running tests built with swiftbuild.

The metadata resource test loads the actual packaged JSON and parses Spanish,
US, and Brazilian numbers. Host test results do not establish that an Android
runner has transferred the resources correctly.
